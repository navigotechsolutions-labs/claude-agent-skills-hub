"""Async Mem0 storage implementation for Upsonic agent framework.

This module provides an asynchronous Mem0 storage backend that supports both:
- Self-hosted Mem0 (via AsyncMemory class from mem0 library)
- Mem0 Platform (via AsyncMemoryClient class from mem0 library)
...
_logger = get_logger("upsonic.storage.mem0.async")


class AsyncMem0Storage(AsyncStorage):
    """Asynchronous Mem0 storage implementation for Upsonic agent framework.
    
    This storage backend uses the Mem0 library for memory persistence.
    It supports both self-hosted Mem0 (AsyncMemory class) and Mem0 Platform
    (AsyncMemoryClient class).
    ...
    Client Creation Modes:
        1. Pass an existing memory_client (AsyncMemory or AsyncMemoryClient instance)
        2. Pass api_key to create an AsyncMemoryClient (Platform)
        3. Pass config to create an AsyncMemory instance (Self-hosted)
    
    Example usage with internal AsyncMemoryClient creation (Platform):
        ```python
        storage = AsyncMem0Storage(api_key="your-api-key")
    ...
    Example usage with internal AsyncMemory creation (Self-hosted):
        ```python
        from mem0.configs.base import MemoryConfig
    ...
    Example usage with external client:
        ...
        # With self-hosted
        memory = AsyncMemory()
        storage = AsyncMem0Storage(memory_client=memory)
    ...
    def __init__(
        self,
        memory_client: Optional[Union["AsyncMemory", "AsyncMemoryClient"]] = None,
        # Platform client (AsyncMemoryClient) parameters
        api_key: Optional[str] = None,
        host: Optional[str] = None,
        org_id: Optional[str] = None,
        project_id: Optional[str] = None,
        # Self-hosted (AsyncMemory) parameters
        ...
        """
        Initialize the async Mem0 storage backend.
        ...
        Args:
            memory_client: Pre-configured AsyncMemory or AsyncMemoryClient instance.
            api_key: Mem0 Platform API key. Creates AsyncMemoryClient if provided.
        ...
        self.memory_client: Union["AsyncMemory", "AsyncMemoryClient"] = self._create_or_use_client(
        ...
        _logger.debug(
            f"Initialized AsyncMem0Storage with "
            f"{'platform' if self._is_platform_client else 'self-hosted'} client"
        )

    def _create_or_use_client(
        self,
        memory_client: Optional[Union["AsyncMemory", "AsyncMemoryClient"]],
        api_key: Optional[str],
        host: Optional[str],
        org_id: Optional[str],
        project_id: Optional[str],
        config: Optional["MemoryConfig"],
    ) -> Union["AsyncMemory", "AsyncMemoryClient"]:
        """
        Create a new async memory client or use the provided one.
        
        Priority order:
        1. Use memory_client if provided
        2. Create AsyncMemoryClient if api_key is provided
        3. Create AsyncMemory if config is provided
        4. Create AsyncMemory with default config
        ...
        Returns:
            AsyncMemory or AsyncMemoryClient instance
        ...
        if memory_client is not None:
            _logger.debug("Using provided async memory client")
        ...
        if effective_api_key:
            _logger.debug("Creating AsyncMemoryClient (platform)")
            ...
            return AsyncMemoryClient(**client_kwargs)
        
        # 3. Create AsyncMemory with config if provided
        if config is not None:
            _logger.debug("Creating AsyncMemory (self-hosted) with custom config")
            return AsyncMemory(config=config)
        
        # 4. Create AsyncMemory with default config
        _logger.debug("Creating AsyncMemory (self-hosted) with default config")
        return AsyncMemory()

    def _check_is_platform_client(self) -> bool:
        """Check if the memory client is a platform client or self-hosted."""
        client_class_name = self.memory_client.__class__.__name__
        return client_class_name == "AsyncMemoryClient"
    ...
    async def close(self) -> None:
        ...
        For AsyncMemoryClient (platform), closes the underlying httpx client.
        For AsyncMemory (self-hosted), performs any necessary cleanup.
        """
        if self._is_platform_client:
            # AsyncMemoryClient has an async_client that may need closing
            if hasattr(self.memory_client, 'async_client'):
                await self.memory_client.async_client.aclose()
        _logger.debug("AsyncMem0 storage closed")
    ...
    async def aupsert_session(
        ...
        """
        Insert or update a session in Mem0 (async).
    ...
    async def aupsert_sessions(
        ...
        for session in sessions:
            try:
                result = await self.aupsert_session(session, deserialize=deserialize)
    ...
    async def aget_sessions(
        ...
        """
        Get multiple sessions from Mem0 (async).
        ...
        try:
            ...
            if session_ids is not None and len(session_ids) > 0:
                ...
                for session_id in session_ids:
                    memory_id = generate_session_memory_id(
                        session_id, self.session_table_name
                    )
                    stored = await self._aget_memory_by_id(memory_id)
                    if stored:
    ...
    async def adelete_sessions(self, session_ids: List[str]) -> int:
        """
        Delete multiple sessions from Mem0 (async).
        ...
        for session_id in session_ids:
            try:
                if await self.adelete_session(session_id):
                    deleted_count += 1
            except Exception as e:
    ...
    async def aupsert_user_memory(
        ...
        """
        Insert or update user memory in Mem0 (async).
    ...
    async def aupsert_user_memories(
        ...
        for memory in user_memories:
            try:
                result = await self.aupsert_user_memory(
                    user_memory=memory,
                ...
                _logger.error(
                    f"Error upserting user memory for {memory.user_id}: {e}"
                )
                raise e
    ...
    async def aget_user_memory(
        ...
        """
        Get user memory from Mem0 (async).
        ...
        try:
            if user_id is not None:
                # Direct lookup by memory_id
                memory_id = generate_user_memory_id(
                    user_id, self.user_memory_table_name
                )
                stored = await self._aget_memory_by_id(memory_id)
    ...
    async def aget_user_memories(
        ...
        """
        Get multiple user memories from Mem0 (async).
        ...
        try:
            ...
            # Deserialize all records
            memory_dicts = [
                deserialize_user_memory_from_mem0(r) for r in paginated_records
            ]
    ...
    async def adelete_user_memory(self, user_id: str) -> bool:
        """
        Delete user memory from Mem0 (async).
    ...
    async def adelete_user_memories(self, user_ids: List[str]) -> int:
        """
        Delete multiple user memories from Mem0 (async).
        ...
        for user_id in user_ids:
            try:
                if await self.adelete_user_memory(user_id):
                    deleted_count += 1
            except Exception as e:
    ...
    async def aclear_all(self) -> None:
        ...
        try:
            ...
            session_records = await self._asearch_memories(filters=session_filters)
            ...
            user_memory_filters = build_user_memory_filters(
                table_name=self.user_memory_table_name
            )
            user_memory_records = await self._asearch_memories(
                filters=user_memory_filters
            )
    ...
    # ======================== Private Async Mem0 API Methods ========================
    # 
    # These methods handle the API differences between:
    # - AsyncMemoryClient (Platform): Supports memory_id on add(), uses text param
    # - AsyncMemory (Self-hosted): Does NOT support memory_id on add(), uses data param
    #
    # For self-hosted AsyncMemory, we use metadata filtering to find records by our 
    # session_id since we cannot specify our own memory_id during creation.

    async def _aadd_memory(
        self,
        memory_id: str,
        content: str,
        ...
        """
        Add a new memory to Mem0 (async).
    ...
    async def _aupdate_memory(
        ...
        """
        Update an existing memory in Mem0 (async).
        ...
        try:
            ...
            actual_memory_id = await self._aresolve_memory_id(memory_id)
    ...
    async def _aget_memory_by_id(self, memory_id: str) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        try:
            return await self._afind_memory_by_upsonic_id(memory_id)
        except Exception as e:
            _logger.debug(f"Error getting memory {memory_id}: {e}")
            return None

    async def _adelete_memory(self, memory_id: str) -> bool:
        """
    ...
    async def _aresolve_memory_id(self, upsonic_memory_id: str) -> Optional[str]:
        """
        Resolve our custom memory_id to the actual Mem0 memory_id (async).
        ...
        """
        memory = await self._afind_memory_by_upsonic_id(upsonic_memory_id)
        if memory:
            return memory.get("id")
        return None

    async def _afind_memory_by_upsonic_id(
        self, upsonic_memory_id: str
    ) -> Optional[Dict[str, Any]]:
        """
        Find a memory record by our custom upsonic_memory_id in metadata (async).
        ...
        try:
            if self._is_platform_client:
                ...
                result = await self.memory_client.get_all(
                    filters={
                        "AND": [
                            {"user_id": self.default_user_id},
    ...
    async def _asearch_memories(
        ...
        """
        Search memories with optional filters (async).
        ...
        try:
            if self._is_platform_client:
                ...
                result = await self.memory_client.get_all(filters=api_filters)
            else:
