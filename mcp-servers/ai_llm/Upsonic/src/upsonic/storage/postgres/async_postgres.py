"""Async PostgreSQL storage implementation for Upsonic agent framework.

This module provides an asynchronous PostgreSQL storage backend using SQLAlchemy
with asyncpg driver. It supports session persistence and user memory storage
...
class AsyncPostgresStorage(AsyncStorage):
    ...
    This storage backend uses SQLAlchemy with asyncpg for async PostgreSQL operations.
    ...
    def __init__(
        ...
        """
        Initialize the async PostgreSQL storage.
        
        Args:
            db_url: PostgreSQL connection URL (e.g., "postgresql+asyncpg://user:pass@host/db").
    ...
    async def table_exists(self, table_name: str) -> bool:
        ...
        async with self.async_session_factory() as sess:
            return await ais_table_available(
                sess, 
                table_name, 
                self.db_schema
            )
    ...
    async def _create_all_tables(self) -> None:
        ...
        if self.create_schema_flag and self.db_schema != "public":
            async with self.async_session_factory() as sess, sess.begin():
                await acreate_schema(sess, self.db_schema)
    ...
    async def _get_or_create_table(
        ...
        async with self.async_session_factory() as sess:
            table_available = await ais_table_available(
                sess, 
                table_name, 
                self.db_schema
            )
    ...
    async def aget_usage(
        ...
        """
        Get the aggregated usage for a specific session (async).
        ...
        table = await self._get_session_table()
        if table is None:
            return None
        ...
        try:
            async with self.async_session_factory() as sess:
                ...
                result = (await sess.execute(stmt)).scalar_one_or_none()
    ...
    async def _get_generic_model_table(self, collection: str, create_if_not_found: bool = True) -> Table:
        ...
        async with self.async_session_factory() as sess:
            table_exists = await ais_table_available(sess, table_name, self.schema_name)
            ...
            async with self.engine.begin() as conn:
                await conn.run_sync(
                    lambda sync_conn: self._metadata.reflect(
                        bind=sync_conn, schema=self.schema_name, only=[table_name]
                    )
                )
    ...
    async def aupsert_model(
        ...
        import time
        from upsonic.storage.postgres.utils import sanitize_postgres_dict
