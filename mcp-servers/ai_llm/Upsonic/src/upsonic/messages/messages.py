...
class BaseToolReturnPart:
    ...
    In case the tool call id is not provided by the model, Upsonic will generate a random one.
    """
...
class RetryPromptPart:
    ...
    * a tool raised a [`ModelRetry`][upsonic.utils.package.exception.ModelRetry] exception
    * no tool was found for the tool name
    * the model returned plain text when a structured response was expected
    * Pydantic validation of a structured response failed, here content is derived from a Pydantic
      [`ValidationError`][pydantic_core.ValidationError]
    * an output validator raised a [`ModelRetry`][upsonic.utils.package.exception.ModelRetry] exception
    """
...
"""A message part sent by Upsonic to a model."""


@dataclass(repr=False)
class ModelRequest:
    """A request generated by Upsonic and sent to a model, e.g. a message from the Upsonic app to the model."""
...
ModelResponsePartTypeAdapter: pydantic.TypeAdapter[list[ModelResponsePart]] = pydantic.TypeAdapter(
    list[ModelResponsePart],
    config=pydantic.ConfigDict(defer_build=True, ser_json_bytes='base64', val_json_bytes='base64'),
)
"""Pydantic TypeAdapter for (de)serializing lists of model response parts."""

BinaryContentTypeAdapter: pydantic.TypeAdapter[list[BinaryContent]] = pydantic.TypeAdapter(
    list[BinaryContent],
    config=pydantic.ConfigDict(defer_build=True, ser_json_bytes='base64', val_json_bytes='base64'),
)
"""Pydantic TypeAdapter for (de)serializing lists of BinaryContent (images/files)."""
