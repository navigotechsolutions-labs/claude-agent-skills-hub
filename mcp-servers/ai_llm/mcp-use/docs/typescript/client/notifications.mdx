...
You can set a default `onNotification` handler at the client level (root config or the second argument to `new MCPClient(config, options)`); all servers will use it unless they override it. You can also register a notification handler per session using the `on` method:
...
- [`MCPClient` API reference](/typescript/api-reference/client/mcp-client) - Server config fields, connection types, and transport options
