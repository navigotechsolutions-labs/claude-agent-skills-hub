...
By default, the CLI deploys from GitHub. It can help you connect the Manufact GitHub App, create a repository when needed, and stream build logs. After the first successful deploy, the CLI writes `.mcp-use/project.json` so later `mcp-use deploy` runs update the same cloud server.
